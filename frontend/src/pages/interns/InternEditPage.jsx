...
import { getInternById, updateIntern } from "../../services/internApi";
import { getDepartments } from "../../services/departmentApi";
import { validateInternForm } from "../../utils/validation";
...
const InternEditPage = () => {
  ...
  useEffect(() => {
    ...
    getInternById(id)
      .then((res) => {
        const intern = res.data?.intern || res.data || null;
        if (!intern) {
  ...
  const handleChange = (event) => {
    ...
    setForm((prev) => {
      ...
      next.endDate = calculateEndDate(next.startDate, next.internshipDuration);
      return next;
    });
    setFieldErrors((prev) => {
      if (!prev[name] && !(name === "startDate" && prev.endDate) && !(name === "internshipDuration" && prev.endDate)) return prev;
      const next = { ...prev };
      delete next[name];
  ...
  const handleSubmit = async (event) => {
    ...
    const validationErrors = validateInternForm(form);
    ...
    try {
      ...
      setError("Could not update intern. Please try again.");
    } finally {
  ...
  return (
    <AdminLayout pageTitle="Edit Intern">
        ...
        {loadingIntern ? (
          ...
          <Card className="border-slate-200 shadow-sm">
            <CardHeader className="border-b border-slate-100 px-6 py-5">
              <CardTitle className="text-lg font-semibold text-slate-800">Edit Intern</CardTitle>
            </CardHeader>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                {fieldErrors.internshipDuration && (
                  <p className="text-xs text-red-600">{fieldErrors.internshipDuration}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-end">
                  <Button type="submit" disabled={isSubmitting}>
                    {isSubmitting ? "Updating..." : "Update Intern"}
                  </Button>
                </div>
              </form>
            </CardContent>
